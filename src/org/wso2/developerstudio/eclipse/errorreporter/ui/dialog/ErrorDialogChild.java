 ...
 *
 */
public class ErrorDialogChild extends IconAndMessageDialog {
	/**
	...
	private void createSupportArea(Composite parent) {
		...
		if(!provider.validFor(statusI)){
			return;
		}

		Composite supportArea = new Composite(parent, SWT.NONE);
		provider.createSupportArea(supportArea, statusI);
	...
	@Override
	public int open() {
		if (!AUTOMATED_MODE && shouldDisplay(statusI, displayMask)) {
			return super.open();
		}
		setReturnCode(OK);
		return OK;
	...
	public static int openError(Shell parentShell, String title,
		...
		ErrorDialogChild dialog = new ErrorDialogChild(parentShell, title, message,
	...
	private boolean listContentExists() {
		return listContentExists(statusI, shouldIncludeTopLevelErrorInDetails);
	}
